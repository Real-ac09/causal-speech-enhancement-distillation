#!/usr/bin/env python3
"""Run frozen Recipe-5b balanced-sampling ablation and evaluation."""
...
CONFIG = Path(
    "configs/v17/utility_safety_recipe5b_balanced_seed17045.yaml"
)
CHECKPOINTS = Path(
    "checkpoints/v17/utility_safety_recipe5b_balanced_seed17045"
)
RESULTS = Path("results/v17/recipe5b_selection")
RECORD = Path("results/v17/recipe5b_execution_record.json")
...
def _run(command: list[str], record: dict, stage: str) -> None:
    ...
    _write(record)
    print(f"\n=== V17 Recipe 5b: {stage} ===", flush=True)
    ...
    if result.returncode:
        raise RuntimeError(
            f"Recipe-5b stage {stage} failed with code {result.returncode}"
        )
...
def main() -> None:
    ...
    frozen = [
        CONFIG,
        Path("configs/v17/frozen_recipe5b_protocol.yaml"),
    ...
    record = {
        ...
        "stages": {
            "recipe5_postmortem": {
                "status": "complete",
                "diagnosis": (
                    "balanced_sampling_ablation_before_architecture_change"
                ),
            },
            "cpu_cuda_preflight_inherited": {
                "status": "complete",
                "model_and_losses_unchanged": True,
            },
        },
        "error": None,
    }
    ...
    try:
        ...
        _run(
            [
                ...
                args.device,
                "--failure-decision",
                "stop_recipe5b_and_build_causal_statistics_controller",
            ],
            record,
        ...
        _write(record)
        print(
            f"\nV17 Recipe 5b complete: {record['decision']}\n"
