...
selection="${project_root}/configs/v15/dns_cross_domain_dev_assets.json"
...
include_paths="$(
    jq --raw-output \
        '[.clean_assets[].path, .noise_assets[].path] | join(",")' \
        "${selection}"
)"
...
LD_LIBRARY_PATH=/home/mohamedb/miniconda3/envs/cnvqg/lib \
    ...
    "${project_root}/scripts/prepare_v15_cross_domain_dev.py" \
    --dataset-root "${dataset_root}" \
    --selection-output "${selection}" \
    --output-dir "${project_root}/data/processed/dns_cross_domain_dev" \
    --materialize
