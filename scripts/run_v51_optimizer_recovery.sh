...
mkdir -p logs/v51_optimizer_recovery
exec 9>logs/v51_optimizer_recovery/recovery.lock
flock -n 9 || { echo "Another V5.1 recovery run is active" >&2; exit 1; }
exec "$CONDA_BIN" run --no-capture-output -n "$ENV_NAME" \
    python scripts/run_v51_optimizer_recovery.py "$@"
