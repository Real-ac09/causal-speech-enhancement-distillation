#!/usr/bin/env python3
"""Evaluate the frozen V16 oracle-supervised gate on development sets."""
...
GATES = Path("configs/v15/promotion_gates.yaml")
...
def main() -> None:
    ...
    parser.add_argument(
        "--candidate-name",
        default="v16_oracle_gate_seed16040",
    )
    parser.add_argument("--bootstrap-seed", type=int, default=16_040)
    ...
    evaluations = (
        (CROSS_METADATA, args.output_root / "cross_domain_dev"),
        (VOICE_METADATA, args.output_root / "voicebank_dev400"),
    )
    for metadata, output in evaluations:
    ...
    comparisons = (
        (
            CROSS_REFERENCE,
            args.output_root / "cross_domain_dev",
            args.output_root / "cross_domain_comparison",
            args.bootstrap_seed,
        ),
        (
            VOICE_REFERENCE,
            args.output_root / "voicebank_dev400",
            args.output_root / "voicebank_comparison",
            args.bootstrap_seed + 1,
        ),
    )
    for reference, candidate, output, seed in comparisons:
        _run(
            [
                sys.executable,
                "scripts/compare_evaluations.py",
                "--reference",
                f"candidate_a={reference}",
                "--candidate",
    ...
    _run(
        [
            ...
            str(CROSS_REFERENCE),
            "--cross-candidate",
            str(args.output_root / "cross_domain_dev"),
            "--voice-reference",
            str(VOICE_REFERENCE),
            "--voice-candidate",
            str(args.output_root / "voicebank_dev400"),
            "--output-dir",
            str(args.output_root / "gate"),
            "--bootstrap-samples",
            "20000",
            "--bootstrap-seed",
            str(args.bootstrap_seed + 3),
        ]
    )
