...
mkdir -p logs/v11
exec 9>logs/v11/decoder.lock
flock -n 9 || { echo "Another V11 decoder stage is active" >&2; exit 1; }
pgrep -f '[s]cripts/train.py' >/dev/null && {
  echo "An existing training job is active" >&2
  exit 2
}
exec "$CONDA_BIN" run --no-capture-output -n "$ENV_NAME" \
  python scripts/run_v11_decoder_stage.py "$@"
