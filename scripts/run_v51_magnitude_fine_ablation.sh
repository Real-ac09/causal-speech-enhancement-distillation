...
ROOT_OUT="results/v51_optimizer_recovery/v51_recovery_main/magnitude_fine_ablation"
...
exec 9>logs/v51_optimizer_recovery/magnitude_fine_ablation.lock
flock -n 9 || { echo "Another V5.1 fine magnitude ablation is active" >&2; exit 1; }

candidate_args=()
for scale in 0.900 0.925 0.950 0.975; do
    label="magnitude_${scale/./_}"
    ...
    if [[ ! -f "$output/summary.json" || ! -f "$output/per_file_metrics.csv" ]]; then
        "$CONDA_BIN" run --no-capture-output -n "$ENV_NAME" \
            ...
            --magnitude-residual-scale "$scale"
    fi
...
echo "Fine magnitude ablation complete: $ROOT_OUT/comparison/comparison.json"
