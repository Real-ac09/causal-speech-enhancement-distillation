#!/usr/bin/env python3
"""Run V17 recipe 4 feature-controller training and learnability."""
...
CONFIG = Path("configs/v17/feature_recipe4_seed17043.yaml")
CHECKPOINTS = Path("checkpoints/v17/feature_recipe4_seed17043")
RESULTS = Path("results/v17/recipe4_learnability")
RECORD = Path("results/v17/recipe4_execution_record.json")
...
def _run(command: list[str], record: dict, stage: str) -> None:
    ...
    if result.returncode:
        raise RuntimeError(
            f"Recipe-4 stage {stage} failed with code {result.returncode}"
        )
...
def main() -> None:
    ...
    frozen = [
        CONFIG,
        Path("configs/v17/frozen_recipe4_protocol.yaml"),
        ...
        Path("src/cnvqg/models/causal_feature_cumulative_gate_v17.py"),
    ...
    record = {
        ...
        "stages": {
            "causality_gradient_native_smoke": {"status": "complete"},
            "cuda_training_smoke": {
                "status": "complete",
                "checkpoint": (
                    "checkpoints/v17/feature_recipe4_smoke/epoch_001.pt"
                ),
            },
        },
        "error": None,
    ...
    try:
        ...
        _run(
            [
                ...
                args.device,
                "--failure-decision",
                "stop_recipe4_and_reassess_oracle_predictability",
            ],
            record,
        ...
        _write(record)
        print(
            f"\nV17 recipe 4 complete: {record['decision']}\n"
