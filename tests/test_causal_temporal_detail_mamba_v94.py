...
from cnvqg.models.causal_temporal_detail_mamba_v94 import (
    CausalTemporalDetailMambaV94,
)
from cnvqg.models.factory import build_model


def _small(**overrides) -> CausalTemporalDetailMambaV94:
    kwargs = dict(
        variant="student",
        detail_channels=16,
        half_channels=24,
    ...
    return CausalTemporalDetailMambaV94(**kwargs)


def test_factory_caps_and_temporal_kernels() -> None:
    student = build_model(
        {"architecture": "cnvqg_v94", "variant": "student", "use_mamba": False}
    )
    teacher = CausalTemporalDetailMambaV94(variant="teacher", use_mamba=False)
    assert student.parameter_count() <= 1_100_000
    assert teacher.parameter_count() <= 2_700_000
    assert student.encoder.stem_conv.conv.kernel_size == (5, 3)
    assert student.decoder.detail_fuse[0].conv.kernel_size == (5, 3)
    assert _small()(torch.randn(1, 1, 1600)).encoder_features.shape[-2] == 129
